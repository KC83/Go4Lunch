...
public class ListWorkmatesTest {
    ...
    public void startMainActivityFromHomeScreen() throws UiObjectNotFoundException {
        ...
        UiObject listButton = mDevice.findObject(new UiSelector().resourceId(Utils.BASIC_PACKAGE+":id/navigation_workmates"));
        listButton.click();
    ...
    public void testShowList() throws UiObjectNotFoundException {
        ...
        UiObject list = mDevice.findObject(new UiSelector().resourceId(Utils.BASIC_PACKAGE+":id/list_workmates"));
    ...
    public void testClickOnElement() throws UiObjectNotFoundException {
        ...
        if (list.getChildCount() > 0) {
            for (int i=0; i<list.getChildCount(); i++) {
                UiObject item = list.getChild(new UiSelector()
                        .resourceId(Utils.BASIC_PACKAGE+":id/item_workmate_title")
                        .enabled(true)
                        .instance(i));

                if (item.getText() != null) {
                    boolean isPlaceChosen = true;
                    if (item.getText().contains(Utils.PLACE_DONT_DECIDED)) {
                        isPlaceChosen = false;
                    }

                    // If the workmates chose a place to go
                    if (isPlaceChosen) {
                        ...
                        // Check if we are still on the same page (WorkmatesFragment)
                        testShowList();
                    }
                }
            }
